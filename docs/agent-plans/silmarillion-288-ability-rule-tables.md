...
- **#285** — *Drift detect: extend ReportUnknowns beyond polymorphic discriminators*. The umbrella for extending the existing CDN drift channel (`IUnknownDiscriminator` / `ReportUnknowns` reported via `IDiagnosticsSink`, exercised daily by [`.github/workflows/cdn-drift-check.yml`](../../.github/workflows/cdn-drift-check.yml)) to cover two adjacent silent-drift channels: free-form-string method-call prefixes (Channel A) and unknown JSON field names on POCOs via `[JsonExtensionData]` (Channel B). The work this issue surfaced on `AbilityKeyword` (5 of 8 fields round-tripping lossy) is exactly Channel B's motivating case — out of scope for #288 itself, owned by #285.
