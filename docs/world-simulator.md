...
**Implementation status:** pending. Lands in a coupled PR with Call 3 (see Call 3's coupling note — Call 1 without Call 3 turns lazy modules' alarm-fire paths into replayed-event blasts on every Mithril restart). The audit (2026-05-22) enumerated six category-(a) `ModuleGate.WaitAsync` sites that retire in the migration: `GardenIngestionService:122`, `FavorIngestionService:126`, `VendorIngestionService:102`, `GourmandIngestionService:90`, `PlayerLogIngestionService:147`, `ItemCollectionTracker:141`. The three Legolas category-(b) gates (`AutoOverlayCoordinator`, `ForegroundFocusGate`, `OverlayController`) survive as UI-hydration gates under the narrowed scope. The `ModuleGate` primitive itself stays; its retirement is deferred to Phase 5 (see #700).
...
**Implementation status:** must land **at-or-before** Call 1 in a coupled PR. Today's sinks (`Samwise.AlarmService.Fire`, `Gandalf.TimerAlarmService.OnTimerReady`) do not mode-gate; combined with eager-always state subscription from Call 1, lazy modules' alarm-fire paths would attach at startup and fire on replayed events at every Mithril restart. `Gandalf.ShiftAlarmService` is currently wall-clock-driven (`DispatcherTimer` against `IGameClock` reading `TimeProvider.System`) and is NOT on Call 3's initial sink list; when #613 (Gandalf scheduler collapse) migrates `ShiftAlarmService` to consume `CalendarTimeAdvanced` events, the mode-gating ships as part of that migration — by the principle, not as a follow-on.
...
**Decision:** the shared `PendingCorrelator<TKey,TReq>` primitive in `Mithril.Shared.Correlation` stays world-sim-unaware. Its constructor continues to accept a `TimeProvider`. Cross-world correlator consumers (today only `IInventoryView`) construct a thin `TimeProvider` subclass whose `GetUtcNow()` returns the view-layer `IViewClock.Now` — `max(lastPlayerFrame, lastChatFrame)` per the Q5 resolution. `DrainStale` calls happen inline from bus-subscribe handlers, never on a wall-clock timer; this is load-bearing for replay determinism — eviction must depend only on observed frame timestamps, not on real elapsed time.

**Rationale:** today's `IInventoryService.PendingCorrelator` (now `IInventoryView`'s post-#602) uses `TimeProvider.System` for TTL eviction. Under replay-from-session-start, the same log + chat produces different eviction sequences depending on how long Mithril has been attached — a determinism violation. Routing TTL through the view clock anchors eviction to the frame trajectory rather than to real elapsed time. Resolves #675.

**Implementation status:** **already implemented** for `IInventoryView` via #602's split (ratified-by-shipping in that PR). This entry documents the pattern for future reference. The Legolas single-world correlator (`ItemCollectionTracker._pendingAdds`) is NOT covered by this ratification; Legolas migrates to PlayerWorld-direct subscription separately (see #699), and any retained correlator there uses PlayerWorld's clock rather than view-clock.
