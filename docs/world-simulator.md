...
**Implementation status:** pending. Lands in a coupled PR with Call 3 (see Call 3's coupling note — Call 1 without Call 3 turns lazy modules' alarm-fire paths into replayed-event blasts on every Mithril restart). The audit (2026-05-22) enumerated six state-subscription gates that retire in the migration (each calls `ModuleGate.WaitAsync` to defer L1-driver subscription and state-machine work until tab activation): `GardenIngestionService:122`, `FavorIngestionService:126`, `VendorIngestionService:102`, `GourmandIngestionService:90`, `PlayerLogIngestionService:147`, `ItemCollectionTracker:141`. The three Legolas UI-hydration gates (`AutoOverlayCoordinator`, `ForegroundFocusGate`, `OverlayController` — they install overlay windows and Win32 dispatcher hooks that genuinely depend on the view being constructed) survive under the narrowed scope. The `ModuleGate` primitive itself stays; its retirement is deferred to Phase 5 (see #700).
