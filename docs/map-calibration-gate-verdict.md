...
Local NCC deviation is *symmetric* — it fires equally whether the screenshot added content the texture lacks, or the texture has content the screenshot lost. Those are opposite cases. An **icon** is screenshot-side detail over smooth texture (`va` high, `vb` low). The reverse — smooth screenshot over detailed texture (`va` low, `vb` high) — is **obscured / removed** content: fog-of-war, and the Kur **ice/bubble patch that renders as a flat grey blob** in the screenshot while the texture art has fine circle detail. The grey blob is a non-icon deviation source, and the per-window variances `va` (screenshot) / `vb` (texture) are already computed, so the two cases are separable for free.

`--added-only` gates deviation on screenshot-side structure (only `va`-high mismatches flag; `va`-low "obscured" windows are treated as a match). On Kur it **removes the grey blob at the source** — deviation foreground halves (low-NCC 10.8% → 4.8%), structure blobs 9 → 2, the left-centre yellow deviation blob vanishes — while icons survive (Eltibule still 10 inliers, Kur still 12; both solves unchanged). It is *restyle-robust* (keys on the variance asymmetry, not a hardcoded fog colour) and is the **principled unified filter for both fog and "removed" terrain features** — the answer to the earlier fog question, with the grey blob as the available proxy (the study screenshots are fully explored, 0 fog).

Caveats: it did **not** independently improve the solve here — the deviation-rim mask + type-floor already absorbed the grey blob, so `--added-only` is a cleaner *front-end* (drops obscured content before it ever becomes a candidate), not an extra robustness win on these two areas. And true fog-of-war still wants a partially-fogged screenshot to confirm — *verification owed*, the grey blob only proves the asymmetry mechanism, not the fog case specifically. **Engine carry-over: compute the deviation as added-only (gate on screenshot-side variance) so fog / obscured terrain never enters the candidate set — strictly cleaner than classifying it out downstream.**
