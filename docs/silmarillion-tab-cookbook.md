...
**Migrated (#318 slice 4): ItemKeyword (#270)** — recipe-detail keyword slots ("any Crystal", "Main-Hand Item") now surface as the "Keyword ingredients" section, each a "View all N →" provenance popup fed `IReferenceDataService.ItemsByRecipeKeywordSlotWithReason` directly (materialized once via the canonical `ItemKeywordIndex` — no query re-derivation, so the count cannot diverge; this dissolved the exact dual-derivation fault where `ItemKeywordQueryMapper` failed whole-slot on `MinTSysPrereq:`/`SkillPrereq:` keys the keyword index nonetheless matched). The `ItemKeyword` synthetic `EntityKind`, its `EntityRef` factories, `ItemKeywordKindTarget` (+ DI registration), `ItemKeywordQueryMapper`, and the `mithril://silmarillion/ItemKeyword/…` route were deleted. Single-reason relationship (`KeywordMatch`) ⇒ the popup collapses to a flat list (#318 Discipline). The single-keyword Items filter-pivot uses on the Abilities/NPCs tabs (a legitimate 1:1 per the chip-vs-popup rule, distinct from this fan-out) degraded to non-navigable plain-text chips — they shared the now-deleted `EntityKind`; restoring their navigability via a dedicated Items-side single-keyword filter-pivot kind is tracked in #327.
