...
2. **When fan-out cardinality is unbounded but the *direct* refs are still useful, cap the chip strip and offer a deep-link to the filtered destination tab.** Two implementations of this idea ship today; **the choice between them is a per-section design decision, not a tab-wide one** — a single tab can use both. Settings live in [`SilmarillionSettings`](../src/Silmarillion.Module/SilmarillionSettings.cs); follow that file as the template if your tab needs persistent settings (`SchemaVersion` stamped from day one, `INotifyPropertyChanged`, source-gen STJ context, debounced autosave via `AddMithrilSettings<T>`).

   **Pattern A — overflow-only pill.** [`ItemsTabViewModel.BuildConsumedByChips`](../src/Silmarillion.Module/ViewModels/ItemsTabViewModel.cs) returns the first `UsedInChipCap` (default 12) chips plus a `+{N-cap} more →` pill **only when** count > cap. The pill is a *signal that the strip is incomplete* — its absence means the strip is the complete answer. Used by Items "Used in" (#273) and Effects "Required by abilities" (#298). Renders as a regular `EntityChip` styled to match the strip; visually integrated with the chips, semantically equivalent to "and more like these." Carries the source entity's icon for visual continuity.

   **Pattern B — always-visible action shortcut.** [`AreaDetailViewModel.BuildNpcChips`](../src/Silmarillion.Module/ViewModels/AreaDetailViewModel.cs) returns the first `UsedInChipCap` chips plus an always-visible shortcut `View all {N} in <Tab> →` regardless of count. The shortcut is a *separate affordance for "go work with the full set in master-list view"*. Used by Areas "NPCs in this area" (#310). Renders as `ActionChip` (transparent fill, ghost border, leading `ListFilter` icon) so the user can tell at a glance that this is "navigate to filtered view," not "open this specific entity." See *Shared.Wpf helpers* below.

   **Discriminator — pick Pattern A vs Pattern B per section, not per tab:**

   | Question about the destination tab | Pattern |
   |---|---|
   | *"Is the destination just a longer list of the same chips?"* (no extra sort / filter facets / query columns) | **A** — the strip already is the destination, no shortcut earns its visual cost on small clusters |
   | *"Does the destination offer structure the strip can't?"* (queryable columns, multi-facet filters, sortable headers, additional metadata per row) | **B** — the shortcut has value at any cardinality because the destination view does more than show more |
   | *"Is the cluster typically tiny (1-3 entries)?"* | Lean **A** even when destination is rich — visual overhead of a permanent shortcut on a 2-chip strip is more noise than value |
   | *"Is the cluster typically large (10+) and bounded by a stable, queryable predicate?"* (`AreaName = "..."`, `EffectKeywordReqs CONTAINS "..."`) | Lean **B** — at scale the user wants to refine, and the shortcut is what unlocks refinement |

   Both patterns share infrastructure: same `UsedInChipCap` setting, same synthetic-kind kind-target plumbing, same `IQueryStringValue` row-field machinery. Switching from A to B (or vice versa) on an existing surface is ~10 LoC. **Provisional rule:** the discriminator above is informed by two surfaces (Pattern A) and one (Pattern B). When a third surface picks B, revisit whether the rule still holds or needs sharpening.
...
- **`ActionChip`** — ghost / outline variant of `EntityChip` for "navigate to a filtered view" semantics (transparent fill, subdued border, leading `ListFilter` icon). **Drop-in replacement** where the chip semantically means "go work with this set in the destination tab" rather than "open this specific entity." Same `EntityChipVm` DataContext shape, same `ClickCommand` contract — picking between the two is purely a visual-affordance call. Use this for Pattern B always-visible shortcuts (see *Cross-link chips → cap + deep-link*) so users can tell entity chips and action shortcuts apart at a glance.
