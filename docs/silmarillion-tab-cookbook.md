...
**Migrated (#318 slice 4): NpcByArea** — Areas-tab "NPCs in this area" "View all N →" opens the provenance popup fed `IReferenceDataService.NpcsByAreaWithReason` directly (derived from the same accumulation as `NpcsByArea` so the two cannot diverge); the `NpcByArea` synthetic `EntityKind`, its `EntityRef.NpcByArea` factory, `NpcByAreaKindTarget` (+ DI registration), and its `mithril://silmarillion/NpcByArea/…` route were deleted. Single-reason relationship (`InArea`) ⇒ flat list (#318 Discipline). `ItemKeyword` (#270) remains a synthetic 1:N surface pending migration in the remaining slice-4 surface. **#311 folded in & closed:** Area-detail landmark groups now render through the shared virtualizing `ProvenancePopupWindow`/`ProvenancePopupViewModel` (recycling `VirtualizingStackPanel`, the ~547-row #259 precedent) — there is no separate non-virtualized list path for the full landmark set. Landmark groups are a *provenance-sectioned* popup (one section per landmark `Type`, since Type is genuine provenance — "which kind of landmark"), deliberately **not** flat (contrast the NPCs single-trivial-reason case).
