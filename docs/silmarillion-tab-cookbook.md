...
2. **When fan-out cardinality is unbounded but the *direct* refs are still useful, cap the chip strip and append a navigable summary chip pointing at the filtered destination tab.** [`AreaDetailViewModel.BuildNpcChips`](../src/Silmarillion.Module/ViewModels/AreaDetailViewModel.cs) is the reference implementation: returns the first `UsedInChipCap` (default 12) chips plus an always-visible navigable summary chip `View all {N} in <Tab> →` regardless of count. The summary chip renders as `ActionChip` (transparent fill, ghost border, leading `ListFilter` icon — see *Shared.Wpf helpers* below) so the user can tell at a glance that this is "navigate to filtered view," not "open this specific entity." Settings live in [`SilmarillionSettings`](../src/Silmarillion.Module/SilmarillionSettings.cs); follow that file as the template if your tab needs persistent settings (`SchemaVersion` stamped from day one, `INotifyPropertyChanged`, source-gen STJ context, debounced autosave via `AddMithrilSettings<T>`).

   **Why always-visible (rather than overflow-only):** the summary chip is a *separate affordance for "go work with the full set in master-list view"*, not a "the strip is incomplete" signal. The destination tab offers structure the strip can't (queryable columns, multi-facet filters, sortable headers) — that value exists at any cardinality, not just past the cap. The visual cost (~24px per section on detail pages already several hundred pixels tall) is small; the consistency benefit (one pattern across every reverse-lookup surface, one decision per handoff) is real.

   **History:** an earlier overflow-only-pill pattern (`+{N-cap} more →`, rendered only when count > cap; #273 Items "Used in" / #298 Effects "Required by abilities") was tried before #310 introduced the always-visible chip. PR #312 documented both as coexisting with a per-section discriminator; that was over-engineered. Migration to the standing pattern tracked in #313 (~10 LoC per surface).
...
- **`ActionChip`** — ghost / outline variant of `EntityChip` for "navigate to a filtered view" semantics (transparent fill, subdued border, leading `ListFilter` icon). **Drop-in replacement** where the chip semantically means "go work with this set in the destination tab" rather than "open this specific entity." Same `EntityChipVm` DataContext shape, same `ClickCommand` contract — picking between the two is purely a visual-affordance call. Canonical use: the navigable summary chip on every reverse-lookup cluster (see *Cross-link chips → cap + summary chip*) so users can tell entity chips and filter-pivot shortcuts apart at a glance.
