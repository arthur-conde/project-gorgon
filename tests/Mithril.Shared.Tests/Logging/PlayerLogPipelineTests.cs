...
/// <summary>
/// L0.5 (#532 / #556) classifier + splitter end-to-end tests. The
/// behaviour-preserving refactor gate carried over from
/// <c>PlayerLogActorRouterTests</c>: any test in this file that PASSED on
/// the pre-#556 router must continue to PASS on the post-#556 pipeline
/// pair (classifier + splitter wired together as production registers
/// them). Drives the pair via an in-memory
/// <see cref="IPlayerLogStream"/> and asserts the fan-out + the
/// <c>Raw</c> opt-in behaviour.
/// </summary>
public sealed class PlayerLogPipelineTests
{
    private static (PlayerLogClassifier Classifier, PlayerLogPipeSplitter Splitter) BuildPipeline(
        IPlayerLogStream upstream, Func<bool>? captureRawAccessor = null)
    {
        var classifier = new PlayerLogClassifier(upstream, captureRawAccessor: captureRawAccessor);
        var splitter = new PlayerLogPipeSplitter(classifier);
        return (classifier, splitter);
    }

    [Fact]
    public async Task Pipeline_fans_out_to_three_typed_pipes()
    {
        var upstream = new ScriptedPlayerLogStream();
        var (classifier, splitter) = BuildPipeline(upstream);
        using var _c = classifier;
        using var _s = splitter;
    ...
    {
        // Two pipelines — one with the toggle off (default), one with it on —
        // each fed an identical line; assert the toggle controls Raw.
        var offUpstream = new ScriptedPlayerLogStream();
        var (offC, offS) = BuildPipeline(offUpstream, captureRawAccessor: () => false);
        using var _oc = offC;
        using var _os = offS;
        var offTask = CollectAsync(((ILocalPlayerLogStream)offS).SubscribeAsync, 1);
        ...
        var (onC, onS) = BuildPipeline(onUpstream, captureRawAccessor: () => true);
        using var _nc = onC;
        using var _ns = onS;
        var onTask = CollectAsync(((ILocalPlayerLogStream)onS).SubscribeAsync, 1);
    ...
    public async Task Anomaly_lines_are_counted_without_being_emitted_on_any_pipe()
    {
        ...
        var localTask = CollectAsync(((ILocalPlayerLogStream)splitter).SubscribeAsync, 0, TimeSpan.FromMilliseconds(400));
        var combatTask = CollectAsync(((ICombatActorLogStream)splitter).SubscribeAsync, 0, TimeSpan.FromMilliseconds(400));
        var systemTask = CollectAsync(((ISystemSignalLogStream)splitter).SubscribeAsync, 0, TimeSpan.FromMilliseconds(400));
        ...
        classifier.Counters.Anomaly.Should().BeGreaterThan(0);
    }
    ...
    public async Task Late_subscriber_sees_replay_then_live()
    {
        ...
        {
            await foreach (var item in ((ILocalPlayerLogStream)splitter).SubscribeAsync(firstCts.Token))
                firstSink.Add(item);
        });
        ...
        // Subscribe AFTER the first line was emitted. The pipeline's per-pipe
        // replay buffer should deliver it to this new subscriber.
        var lateTask = CollectAsync(((ILocalPlayerLogStream)splitter).SubscribeAsync, 2);
    ...
    {
        // L1 (#550 PR 1) added the *WithReplayMarker variant; the pipeline's
        ...
        // L0.5 API gains a corresponding pin so future refactors can't
        // silently drop the boundary signal.
        var upstream = new ScriptedPlayerLogStream();
        var (classifier, splitter) = BuildPipeline(upstream);
        using var _c = classifier;
        using var _s = splitter;
        ...
        // pipeline's per-pipe replay buffer accumulates them. Then attach a
        ...
        {
            await foreach (var item in ((ILocalPlayerLogStream)splitter).SubscribeAsync(holderCts.Token))
                holderSink.Add(item);
        });
        ...
        {
            await foreach (var envelope in ((ILocalPlayerLogStream)splitter).SubscribeWithReplayMarkerAsync(lateCts.Token))
            {
    ...
    {
        // #547 race: the classifier's StopRunning previously nulled _runTask
        ...
        // same upstream emission to the new subscriber's pipe channel. Per
        // #556 the orchestration lives in PlayerLogClassifier, so the race
        // is exercised at that layer — the test here drives the classifier
        // directly via the unified-pipe marker subscription rather than
        // the typed pipe (which now goes through the splitter and so
        // wouldn't expose the classifier's own restart race symmetrically).
        var upstream = new BlockingPerSubscriberStream();
        var classifier = new PlayerLogClassifier(upstream);
        using var _c = classifier;

        // First subscriber: receives line 1 then closes.
        var firstCollected = await CollectOneClassifiedThenCloseAsync(
            ((IClassifiedPlayerLogStream)classifier).SubscribeWithReplayMarkerAsync,
            async () =>
            {
        ...
        ((LocalPlayerLogLine)firstCollected[0].Payload).Data.Should().StartWith("ProcessAddItem(A");
        ...
        // emission per push.
        var lateTask = CollectClassifiedAsync(
            ((IClassifiedPlayerLogStream)classifier).SubscribeWithReplayMarkerAsync,
            expected: 5, timeBudget: TimeSpan.FromSeconds(2));
        ...
        collected.Should().HaveCount(1,
            because: $"observed payload Data values: [{string.Join(", ", collected.Select(x => $"\"{((LocalPlayerLogLine)x.Payload).Data[..Math.Min(30, ((LocalPlayerLogLine)x.Payload).Data.Length)]}\""))}]");
        ((LocalPlayerLogLine)collected[0].Payload).Data.Should().StartWith("ProcessAddItem(B");
    }

    /// <summary>
    /// Subscribe once via the marker variant, run <paramref name="trigger"/>
    /// while subscribed, collect exactly one item, then exit the iteration
    /// (triggering the classifier's no-subscribers StopRunning path).
    /// </summary>
    private static async Task<List<LogEnvelope<IClassifiedPlayerLogLine>>>
        CollectOneClassifiedThenCloseAsync(
            Func<CancellationToken, IAsyncEnumerable<LogEnvelope<IClassifiedPlayerLogLine>>> subscribe,
            Func<Task> trigger)
    {
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(5));
        var result = new List<LogEnvelope<IClassifiedPlayerLogLine>>();
            ...
            TimeSpan? timeBudget = null)
    {
        ...
        catch (OperationCanceledException) { /* expected when expected exceeds delivered */ }
        return result;
    }
    ...
    /// only one upstream consumer ever reads the line). Subscriber
    /// enumerators stay alive after cancellation until <see cref="Release"/>
    /// is called, so a test can deterministically hold open the prior
    /// RunAsync's `await foreach` while a new subscriber attaches.
    /// </summary>
    private sealed class BlockingPerSubscriberStream : IPlayerLogStream
    {
        ...
        public async IAsyncEnumerable<RawLogLine> SubscribeAsync([EnumeratorCancellation] CancellationToken ct)
        {
            ...
            try
            {
                ...
                // in flight" while StopRunning has already returned.
