...
**Landing review fixes — fresh dispatch every round, no continuity to preserve.** The engineer is stateless across rounds: once it returns, its context is gone, and (per the dispatch rule above) you cannot synchronously resume it. So to land review fixes, dispatch a **new blocking engineer** briefed with the current PR diff plus the specific findings to address — the diff, commits, and findings are a complete handoff, so a fresh engineer loses little. Do not try to "continue the same engineer" to keep its context warm; the only mechanism for that is `SendMessage`, which backgrounds the agent and breaks the synchronous model. Curate each fix-round brief the same way you curate the first one, and verify any concrete claim in it against live code before dispatching.
