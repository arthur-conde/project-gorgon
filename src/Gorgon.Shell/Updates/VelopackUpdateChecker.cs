...
public sealed class VelopackUpdateChecker : IUpdateChecker
{
    ...
    public VelopackUpdateChecker(GorgonUpdateManager holder, IUpdateStatusService status, IDiagnosticsSink diag)
    {
        _holder = holder;
        _status = status;
        _diag = diag;
    ...
    public async Task CheckAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            _status.ReportResult(
                ...
                releaseNotesUrl: $"{GorgonUpdateManager.RepoUrl}/releases/tag/v{targetVersion}");
