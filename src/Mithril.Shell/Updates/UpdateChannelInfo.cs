...
/// <c>AssemblyMetadataAttribute("MithrilUpdateChannel", ...)</c>. CI publishes with
/// <c>-p:MithrilUpdateChannel=selfcontained</c> or <c>fxdep</c>; local F5 builds get the
...
public sealed record UpdateChannelInfo(string Name, string DisplayName)
{
    ...
    public static UpdateChannelInfo FromAssembly(Assembly asm)
    {
        var raw = asm.GetCustomAttributes<AssemblyMetadataAttribute>()
            .FirstOrDefault(a => string.Equals(a.Key, "MithrilUpdateChannel", StringComparison.Ordinal))?
            .Value;
